...
// ============================== VENDORED CODE ==============================
//
// Do NOT edit the Solidity below. It is a verbatim copy of third-party,
// audited source. The only thing this project added is this comment block.
//
//   Upstream   https://github.com/daimo-eth/p256-verifier
//   File       src/P256Verifier.sol
//   Commit     402e7b4ec4b91f941529f43f9270a8727f647c98 (last change to this
//              file, 2023-09-28); repository head at the time of vendoring
//              607d3ec8377a3f59d65eca60d87dee8485d2ebcc
//   Licence    MIT - see ./LICENSE, (c) 2023 Renaud Dubois (Ledger) and
//              (c) 2023 Daimo, Inc
//   Audit      Veridise, October 2023
//   Lineage    Ledger's FreshCryptoLib (rdubois-crypto/FreshCryptoLib)
//
// WHY IT IS HERE. docs/18-tareas-por-fases.md Fase 5 item 2 asks for a
// Solidity fallback for when the RIP-7212 precompile does not answer. Nobody
// on this project is going to hand-write secp256r1 arithmetic: subtle,
// unauditable curve bugs are exactly how signature verifiers get broken. So
// the fallback is somebody else's audited implementation, copied in whole and
// unmodified, and `src/P256.sol` reaches it by ADDRESS, never by import.
//
// WE DO NOT DEPLOY THIS, AND THIS FILE IS NOT COMPILED. It sits outside
// `src`, `test` and `script`, so `forge build` never touches it and no
// compiler pin in foundry.toml has to bend around its hard `pragma solidity
// 0.8.21`. It is here to be READ: the contract this project actually calls is
// the canonical deployment, and a repository that depends on a contract it
// cannot show you is a repository that cannot be reviewed.
//
//   Canonical address  0xc2b78104907F722DABAc4C69f826a522B2754De4
//   Deployed via       the CREATE2 deployer at 0x4e59b448...c0B4956C, so the
//                      address is the same on every chain it reaches
//   Live on Fuji       yes - confirmed by `eth_call` on 13/09/2026: a valid
//                      signature returns 0x..01, a signature over another
//                      digest returns 0x..00, and its high-s twin returns
//                      0x..01 (see the malleability note below)
//
// PROVENANCE, CHECKED RATHER THAN ASSERTED. Compiled with upstream's own build
// settings - solc 0.8.21, via_ir on, 999999 optimizer runs, evm_version
// `paris` - this source reproduces the runtime bytecode deployed at
// 0xc2b7..4De4 on Fuji EXACTLY, byte for byte across all 3484 bytes of code,
// with only the trailing CBOR metadata differing. So the file below is not
// merely "the same project": it is the source of the contract we call. To
// re-check it, build it with those settings and compare against
// `cast code 0xc2b78104907F722DABAc4C69f826a522B2754De4 --rpc-url <fuji>`.
//
// The test suite does not build this file either. It runs the canonical
// runtime bytecode directly - see test/mocks/CanonicalP256Verifier.sol - so
// what the tests exercise is the deployed artifact and not a local rebuild of
// it.
//
// NOTE ON MALLEABILITY. This verifier follows the NIST rule and accepts BOTH
// s and n-s. Confirmed against the live deployment on Fuji. The low-s policy
// is enforced one level up, in `src/P256.sol`, so that it applies to the
// precompile and to this fallback alike.
//
// ===========================================================================
