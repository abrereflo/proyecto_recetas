...
> Firma la cuenta de la farmacia, y hoy esa cuenta es una clave que vive cifrada en el teléfono del mostrador, protegida con una contraseña. No es el diseño final y lo decimos: el diseño es passkey sobre una smart account con paymaster, y esa parte está **construida y probada** —`PasskeyAccount.sol`, su fábrica, `PrescriptionPaymaster.sol` y un relayer propio—. Lo que falta no es escribirla: es **desplegarla y financiar el depósito**. Y no hace falta ningún bundler: `handleOps` del EntryPoint v0.7 es `public` y sin control de acceso, así que el relayer envía directo. Elegimos esto por un motivo operativo muy concreto: el navegador interno de la extensión no da acceso a la cámara, y esta aplicación es un escáner. Lo que la demostración prueba —que la segunda dispensación de la misma receta revierte— no depende de cómo se firma, y el firmante está aislado detrás de un puerto para que el cambio sea acotado.
