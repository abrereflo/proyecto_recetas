...
>
> **Contexto.** El esquema registrado en `contracts/script/RegisterSchemas.s.sol:43` —idéntico en `contracts/script/LocalDemo.sol:24`— es `PharmacyCredential(string pharmacyLicense,string sanitaryRegistryRef,address issuerAuthority,uint64 validFrom,uint64 validUntil)`. Se cita sin espacios a propósito: el uid del esquema se deriva de `keccak256` de esa cadena exacta, de modo que cualquier variante de formato apunta a otro esquema. No tiene campo de tipo, así que el sistema no distingue hoy entre una farmacia independiente, una sucursal de una cadena de venta al público y la farmacia de un centro de salud. Las tres se acreditan igual y dispensan igual. La distinción importa por dos motivos: el régimen de sustancias controladas de [07](07-seguridad-y-cumplimiento.md#d-18) no es el mismo para una oficina de farmacia que para un servicio hospitalario, y la supervisión agregada que este documento promete —agregados por farmacia— necesita saber qué representa cada dirección.
>
> **Estado real del campo `sanitaryRegistryRef`.** Se escribe en la emisión sobre red pública, y en ningún otro sitio. `contracts/script/IssueCredential.s.sol` codifica el payload declarado con `abi.encode` en el orden del esquema, `sanitaryRegistryRef` incluido, tomando el valor de `PHARMACY_SANITARY_REGISTRY_REF` y avisando cuando llega vacío. Lo demás sigue en pie, y es lo que de verdad condiciona las opciones: **nadie lo lee**. No aparece en `apps/`, ni en `packages/`, ni en la lógica del contrato, y el registro nunca decodifica el `bytes data` de la attestation (`contracts/src/IEAS.sol:23`). Y la ruta local no puede ni escribirlo: `MockEAS.attest` (`contracts/test/mocks/MockEAS.sol:41-51`) no tiene parámetro de datos, así que sobre Anvil ningún campo del esquema llega a la attestation. En resumen: hay dato en Fuji, no hay dato en Anvil, y no hay lector en ninguna de las dos.
>
> **Opciones.** (a) Añadir un campo `pharmacyType` al esquema, con un conjunto cerrado de valores acordado con SEDES. (b) No tocar el esquema y derivar el tipo off-chain desde `sanitaryRegistryRef`, poblándolo primero en la emisión. (c) Registrar un esquema distinto por tipo, de modo que el contrato pueda exigir uno u otro según el medicamento. (d) No modelarlo: toda farmacia acreditada es equivalente.
>
> **Recomendación.** (b) para el piloto. La objeción que tenía —que exigía codificar el payload del esquema en la emisión— ya no aplica sobre Fuji, donde `IssueCredential.s.sol` lo codifica. Lo que falta para (b) es lo otro: poblar el campo con un valor real del registro sanitario en vez de la cadena vacía, y escribir el lector, que no existe. A cambio no cambia el uid ni obliga a reemitir bajo otro esquema, y deja la fuente de verdad sobre qué es cada establecimiento en el registro sanitario y no en una afirmación del proyecto. Reservar (c) para cuando entren sustancias controladas ([D-18](07-seguridad-y-cumplimiento.md#d-18)), que es el único caso conocido en que el contrato necesitaría decidir según el tipo. La opción (a) parece la más simple y es la más cara: un cambio de esquema cambia el uid y obliga a reemitir todas las credenciales vigentes.
>
