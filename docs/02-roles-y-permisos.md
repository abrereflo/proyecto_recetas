...
>
> **Estado real del campo `sanitaryRegistryRef`.** Está declarado en el esquema y no se escribe ni se lee en ninguna parte: no aparece en `apps/`, ni en `packages/`, ni en la lógica del contrato, y `contracts/script/SetupCredentials.s.sol` atesta con el payload vacío. Hoy ese campo está vacío en la cadena. Conviene dejarlo escrito porque condiciona las opciones: cualquiera que se apoye en él empieza por poblarlo.
>
