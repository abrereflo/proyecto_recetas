...
>
> **Estado real del campo `sanitaryRegistryRef`.** Está declarado en el esquema y no se escribe ni se lee en ninguna parte: no aparece en `apps/`, ni en `packages/`, ni en la lógica del contrato, y la ruta de acreditación local no puede escribirlo: `MockEAS.attest` (`contracts/test/mocks/MockEAS.sol:41-51`) no tiene parámetro de datos, de modo que ningún campo del esquema llega a la attestation. La `Attestation` real sí lleva `bytes data` (`contracts/src/IEAS.sol:23`), pero el registro nunca la decodifica. Conviene dejarlo escrito porque condiciona las opciones: cualquiera que se apoye en él empieza por poblarlo.
>
