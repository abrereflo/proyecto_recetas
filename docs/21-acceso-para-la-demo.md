...
| # | Obstáculo | Evidencia |
|---|---|---|
| 1 | Las piezas ERC-4337 están construidas pero **no desplegadas**: no hay fábrica, ni paymaster, ni depósito, ni stake en ninguna cadena. La disponibilidad de un proveedor de bundler dejó de ser un obstáculo —el proyecto no usa uno— pero desplegar y financiar sigue pendiente | `contracts/src/PasskeyAccount.sol`, `PasskeyAccountFactory.sol`, `PrescriptionPaymaster.sol`; `env.example`, `PRESCRIPTION_PAYMASTER_ADDRESS` y `PASSKEY_ACCOUNT_FACTORY_ADDRESS` vacías |
...
| 4 | [D-02](01-arquitectura.md) —quién financia el paymaster— sigue abierta, y con ella la política de patrocinio | [01](01-arquitectura.md), bloque D-02; [18](18-tareas-por-fases.md), Fase 5, tarea «Financiar el paymaster» sin marcar |

La Fase 5 de [18](18-tareas-por-fases.md) agrupa ese trabajo en ocho tareas, de las que seis están cerradas: la cuenta, la ruta de respaldo P-256, la verificación WebAuthn en cadena, el relayer propio, la política de patrocinio y el mensaje de rechazo. Lo que queda abierto es el uso —ninguna pantalla llama al puerto de passkey— y financiar el paymaster, y el criterio de salida es «una emisión completa sin que el médico posea AVAX». Ese criterio no se alcanza a tiempo para la demo, y forzarlo pondría en riesgo lo único que la demo tiene que demostrar: que el segundo intento de dispensar revierte.

> **La contradicción con la documentación es real y está declarada.** `docs/08-stack-y-entorno.md:16` afirma «Ningún médico instalará una extensión» como motivo de la elección de WebAuthn frente a una extensión. El código de hoy exige exactamente esa extensión, y sus dos adaptadores lo dicen en un `TODO` que apunta a D-04 y a la Fase 5 (`apps/doctor/src/infrastructure/signer/eip1193-signer.adapter.ts`, en el `TODO` de su cabecera; `apps/pharmacy/src/infrastructure/signer/eip1193-signer.adapter.ts`, en el mismo sitio). Este documento no resuelve la contradicción: la acota, la fecha y le pone una condición de salida en [D-31](#d-31).
...
> Todavía no. La passkey y el paymaster están construidos y pasan sus pruebas —la cuenta, el verificador WebAuthn en cadena, el paymaster y un relayer propio—, pero no hay nada desplegado y ninguna pantalla los llama, así que decir que «ya está» sería falso. Para no prometer lo que no corre, hoy firmamos con una extensión estándar y lo decimos abiertamente. Lo que la demo prueba —que la segunda dispensación revierte— no depende de cómo se firma, y la pieza que sí depende ya está aislada detrás de un puerto para que el cambio sea acotado.
...
>
> **Contexto.** El acceso de la demo y del primer piloto se apoya en una extensión de navegador, con la cuenta del propio profesional pagando el gas. El modelo objetivo —smart account ERC-4337 con passkey y paymaster— está especificado en [01](01-arquitectura.md) y planificado como Fase 5 en [18](18-tareas-por-fases.md), y su código ya existe —cuenta, paymaster, cliente de passkey y relayer propio, todos con pruebas—, pero depende de tres cosas que hoy no se cumplen: desplegar la fábrica y el paymaster, una resolución de [D-02](01-arquitectura.md) sobre quién financia el patrocinio, y una ruta de escritura que use `PasskeyPort` en lugar de `SignerPort` en el composition root del médico. Sin una condición escrita, el andamiaje se queda por inercia: es lo que funciona, y lo que funciona no se toca.
>
...
| De la Fase 5 quedan sin marcar el registro de passkey en la interfaz y financiar el paymaster; las otras seis tareas están cerradas | Lectura de `docs/18-tareas-por-fases.md`, sección «Fase 5» |
