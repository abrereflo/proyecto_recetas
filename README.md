...
A verifiable e-prescription system built on Avalanche Fuji (chainId 43113), the testnet of Avalanche's EVM-compatible C-Chain. Avalanche is an independent L1, not an Ethereum L2; what the project borrows from Ethereum are its standards, which run on any EVM. Doctors sign prescriptions with a wallet connected in the browser. The passkey path — an ERC-4337 smart account with a paymaster covering gas — is built and tested: `PasskeyAccount`, its factory, `PrescriptionPaymaster`, a passkey client in the doctor app and a relayer of the project's own in `services/api`. None of it is deployed, and no application calls it yet, so it is not what a demo shows. Pharmacies scan a QR code, verify the prescriber's credential on-chain through the Ethereum Attestation Service, and dispense. A second attempt to dispense the same prescription reverts. Clinical content stays encrypted off-chain; only a content hash and a per-prescription salted commitment go on-chain.
