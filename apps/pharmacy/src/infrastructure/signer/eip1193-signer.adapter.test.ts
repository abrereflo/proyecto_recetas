 ...
 *
 * The doctor's `errorCode` already walked the `cause` chain recursively; this
 * one only read the top level and returned `undefined` for a wrapped code.
 * Fixed here so a provider that wraps 4902 one level deep in `cause` still
 * reaches the fallback, in both apps.
 */
...
describe('ensureChain — the 4902 fallback (Corte 1)', () => {
  ...
  it('detects 4902 wrapped one level deep in cause, driving the extension to add the chain', async () => {
    const { provider, calls } = providerRejecting(
      [
        () => Promise.reject({ cause: { code: UNRECOGNISED_CHAIN } }),
        () => Promise.resolve(null),
      ],
      () => Promise.resolve(null),
    );
...
describe('errorCode — walking the cause chain', () => {
  it('finds a code wrapped one level deep in cause', () => {
    expect(errorCode({ cause: { code: UNRECOGNISED_CHAIN } })).toBe(UNRECOGNISED_CHAIN);
  });

  it('still finds a top-level code, unchanged', () => {
    expect(errorCode({ code: USER_REJECTED })).toBe(USER_REJECTED);
  });

  it('returns undefined for a value with no code anywhere in the chain', () => {
    expect(errorCode({ cause: { message: 'no code here' } })).toBeUndefined();
  });
});
