 ...
 * contracts/script/Deploy.s.sol runs, and an app that crashes while loading
 * its own bundle cannot render the message explaining why. The result is
 * therefore a value the UI can branch on; `loadPharmacyConfig()` is the
 * throwing variant for call sites that already know the config is good.
 *
 * The Vite dev server reads the root `.env` because `vite.config.ts` points
 * `envDir` at the workspace root.
 */
...
export type PharmacyConfigResult =
  | { ok: true; config: PharmacyConfig }
  | { ok: false; error: ConfigurationError };

/** Shape of the environment this app reads. Every value arrives as a string. */
export type PharmacyEnv = Record<string, string | boolean | undefined>;
...
} as const satisfies Record<keyof PharmacyConfig, Rule>;

function read<T>(env: PharmacyEnv, rule: Rule): T {
...
export function loadPharmacyConfig(env: PharmacyEnv = importMetaEnv()): PharmacyConfig {
  return {
...
export function readPharmacyConfig(env: PharmacyEnv = importMetaEnv()): PharmacyConfigResult {
  try {
    return { ok: true, config: loadPharmacyConfig(env) };
  } catch (error) {
...
function importMetaEnv(): PharmacyEnv {
  return import.meta.env as unknown as PharmacyEnv;
}
 ...
 */
export const pharmacyConfigResult: PharmacyConfigResult = readPharmacyConfig();
