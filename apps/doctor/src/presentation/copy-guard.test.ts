 ...
 *
 * Mirrors apps/pharmacy/src/presentation/copy-guard.test.ts. src/copy-guard.test.ts
 * covers the core, where whole-file scanning is impossible because the adapters
 * legitimately use protocol identifiers that contain a forbidden word; nothing
 * in this layer does, so whole files are scanned here.
 *
...
describe('no navigation state reaches the URL', () => {
  ...
  // decryption key and the draft carries the patient identifier, so a router
  // would leak clinical content into the address bar, the history and every
  // later `Referer` header.
  ...
  it('declares no router dependency in package.json', async () => {
    const manifest = JSON.parse(
      await readFile(resolve(PRESENTATION, '..', '..', 'package.json'), 'utf8'),
    ) as { dependencies?: Record<string, string>; devDependencies?: Record<string, string> };

    const installed = Object.keys({ ...manifest.dependencies, ...manifest.devDependencies });
    expect(installed.filter((name) => /router|wouter|navigo/i.test(name))).toEqual([]);
  });
});
